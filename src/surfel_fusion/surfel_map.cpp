...
void SurfelMap::image_input( const ros::Time _t, const cv::Mat& _im )
{
    cout << "[SurfelMap::image_input]" << "t=" << _t << "\t";
    cout << "im: " << _im.rows << "x" << _im.cols << "x" << _im.channels() << endl;
    image_buffer.push_back(std::make_pair(_t, _im));
    synchronize_msgs();
}
...
void SurfelMap::depth_input( const ros::Time _t, const cv::Mat& _depthim )
{
    cout << "[SurfelMap::depth_input] " << "t=" << _t << endl;
    constexpr double kDepthScalingFactor = 0.001; //16UC1 has depth vals in mm, I need it in meters, so this constant
    if( _depthim.type() == CV_16UC1 )
    {
        cv::Mat depth_image;
...
int SurfelMap::n_surfels() const
{
    return n_active_surfels() + n_fused_surfels();
}

int SurfelMap::n_active_surfels() const
{
    int n=0;
    // only count those which are updated less than 5 times
    for(int i = 0; i < local_surfels.size(); i++)
    {
        // cout << i << " local_surfels[i].update_times= " << local_surfels[i].update_times << endl;
...
int SurfelMap::n_fused_surfels() const
{
    int n=0;

    // inactive - loop over all poses and count surfels in each
    for(int i = 0; i < poses_database.size(); i++)
    {
        n+= (int)poses_database[i].attached_surfels.size();
    }
    return n;
}


Eigen::MatrixXd SurfelMap::get_surfel_positions() const // return 4XN matrix
{
    int n = n_surfels();
    Eigen::MatrixXd w_X = Eigen::MatrixXd::Ones( 4, n );
    int c = 0;
...
{
    int n = n_surfels();
    Eigen::MatrixXd normals = Eigen::MatrixXd::Ones( 3, n );
    int c = 0;
...
void SurfelMap::camera_pose__w_T_ci__input( ros::Time _t, geometry_msgs::Pose _pose )
{
    if(poses_database.size() > 0)
    {
        if(_t.toNSec() > poses_database.back().cam_stamp.toNSec())
        {
            PoseElement this_pose_element;
            int this_pose_index = poses_database.size();
            this_pose_element.cam_pose  = _pose;
            this_pose_element.loop_pose = _pose;
            this_pose_element.cam_stamp = _t;
            ...
            pose_reference_buffer.push_back(std::make_pair(_t, this_pose_index));
        }
    }
    else
    {
...
void SurfelMap::camera_pose__w_T_ci__input( ros::Time _t, Eigen::Matrix4d& _posemat )
{
    geometry_msgs::Pose _pose;
    pose_eigen2ros( _posemat, _pose );
    camera_pose__w_T_ci__input( _t, _pose );
...
void SurfelMap::push_a_surfel(vector<float> &vertexs, SurfelElement &this_surfel)
{
    ...
    // Making a hexagon centered around the 3d point.
